...
이 문서에서는 Azure에서 Linux VM(가상 머신)을 실행하기 위해 검증된 일단의 사례에 대해 설명합니다. 여기에는 VM 프로비전과 함께 네트워킹 및 저장소 구성 요소에 관한 권장 사항이 포함됩니다. [**이 솔루션을 배포합니다.**](#deploy-the-solution)
...
일반적으로 내부 사용자 또는 고객에게 가장 가까운 Azure 지역을 선택합니다. 그러나 일부 지역에서는 일부 VM 크기를 사용하지 못할 수 있습니다. 자세한 내용은 [지역별 서비스][services-by-region]를 참조하세요. 지정된 지역에서 사용할 수 있는 VM 크기 목록을 보려면 다음 Azure CLI(명령줄 인터페이스) 명령을 실행합니다.
...
[VM 크기를 변경][vm-resize]하여 VM을 규모 확장 또는 축소할 수 있습니다. 규모를 확장하려면 부하 분산 장치 뒤에 둘 이상의 VM을 배치합니다. 자세한 내용은 [N 계층 참조 아키텍처](./n-tier-cassandra.md)를 참조하세요.
...
배포는 [GitHub][github-folder]에서 사용할 수 있습니다. 다음을 배포합니다.
...
2. `single-vm-v2.json` 파일을 열고 큰따옴표 사이에 사용자 이름과 SSH 공개 키를 입력한 다음, 파일을 저장합니다.
...
웹 브라우저에서 이 주소로 이동하면 기본 Apache2 홈 페이지가 표시됩니다.
