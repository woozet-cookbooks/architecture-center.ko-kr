...
데이터베이스 계층에 여러 개의 VM이 존재한다고 해서 자동으로 고가용성 데이터베이스가 되는 것은 아닙니다. 관계형 데이터베이스의 고가용성을 달성하기 위해서는 일반적으로 복제와 장애 조치(failover)를 사용해야 합니다. SQL Server의 고가용성을 달성하기 위해서는 [Always On 가용성 그룹][sql-alwayson]을 사용하는 것이 좋습니다. 
