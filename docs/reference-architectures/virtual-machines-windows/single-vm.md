...
이 아키텍처는 Azure에서 Windows VM을 실행하기 위한 기본 권장 사항을 보여 줍니다. 그러나 단일 실패 지점을 생성하므로 중요한 작업에 단일 VM을 사용하지 않는 것이 좋습니다. 더 높은 가용성을 위해 둘 이상의 부하가 분산된 VM을 배포합니다. 자세한 내용은 [Azure에서 여러 VM 실행][multi-vm]을 참조하세요.
