...
* **클라우드 응용 프로그램**. Azure에 호스팅된 응용 프로그램. 여러 계층이 포함될 수 있고, Azure 부하 분산 장치를 통해 여러 서브넷을 연결합니다. 어플레케이션 인프라 스트럭처에 대한 더 많은 정보가 필요하신경우 , [Windows VM 워크로드 실행][windows-vm-ra] 및 [Linux VM 워크로드 실행 ][linux-vm-ra] 자료를 참고하세요.
