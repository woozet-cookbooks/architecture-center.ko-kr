...
기존의 CRUD 디자인은 데이터 작업에 제한된 비즈니스 논리가 적용된 경우에만 제대로 작동합니다. 개발 도구가 제공하는 스캐폴드 메커니즘은 데이터 액세스 코드를 매우 빠르게 만들 수 있는데, 이 코드는 나중에 필요에 따라 사용자 지정할 수 있습니다.
...
- 결과적 일관성을 추구하기 위한 일반적인 접근 방식은 이벤트 소싱을 CQRS와 함께 사용하는 것입니다. 그러면 쓰기 모델은 명령의 실행으로 구동되는 추가 전용 이벤트 스트림이 됩니다. 이러한 이벤트는 읽기 모델로 작용하는 구체화된 뷰를 업데이트하는 데 사용합니다. 자세한 내용은 [이벤트 소싱 및 CQRS](/azure/architecture/patterns/cqrs#event-sourcing-and-cqrs)를 참조하세요.
...
- 데이터 읽기 성능을 데이터 쓰기 성능과 별도로 세밀하게 조정해야 하는 시나리오(특히 읽기/쓰기 비율이 매우 높고 수평 확장이 필요한 경우). 예를 들어 많은 시스템에서 읽기 작업 수가 쓰기 작업 수보다 몇 배 더 많습니다. 이런 사실을 감안한다면 읽기 모델을 확장하고 쓰기 모델은 하나 또는 소수의 인스턴스에서만 실행하는 것이 좋습니다. 소수의 쓰기 모델 인스턴스는 병합 충돌 발생을 최소화하는 데도 기여합니다.
...
- 전체 시스템에 구현하는 경우. 전체 데이터 관리 시나리오에서 CQRS가 유용할 수 있는 특정 구성 요소가 있지만, CQRS가 필요하지 않은 경우에는 불필요한 복잡성을 상당히 추가할 수 있습니다.
...
- [데이터 일관성 입문서](https://msdn.microsoft.com/library/dn589800.aspx). CQRS 패턴을 사용할 때 읽기 및 쓰기 데이터 저장소 간의 결과적 일관성 때문에 일반적으로 발생하는 문제와 이러한 문제의 해결 방법을 설명합니다.
