...
| 밀도 | 장치 및 시스템 독립성 | 앱 서비스 요금제를 통해 인스턴스당 복수의 앱 지원 | VM당 복수의 서비스 지원 | 서버를 사용하지 않음 <a href="#note1"><sup>1</sup></a> | VM당 복수의 컨테이너 지원 |전용 인스턴스 없음 | VM당 복수의 앱 지원 |
| 최소 노드 개수 | 1 <a href="#note2"><sup>2</sup></a>  | 1 | 5 <a href="#note3"><sup>3</sup></a> | 서버를 사용하지 않음 <a href="#note1"><sup>1</sup></a> | 3 | 전용 노드 없음 | 1 <a href="#note4"><sup>4</sup></a> |
...
| 전용 VNet에 배포 가능 여부 | 지원됨 | 지원됨 <a href="#note5"><sup>5</sup></a> | 지원됨 | 지원됨 <a href="#note5"><sup>5</sup></a> | 지원됨 | 지원되지 않음 | 지원됨 |
...
| 조건 | Virtual Machines | App Service | Service Fabric | Azure 기능 | Azure Container Service | Container Instances | Azure Batch |
|----------|-----------------|-------------|----------------|-----------------|-------------------------|----------------|-------------|
| 로컬 디버깅 | 장치 및 시스템 독립성 | IIS Express, 기타 <a href="#note1b"><sup>1</sup></a> | 로컬 노드 클러스터 | Azure Studio 또는 Azure Functions CLI | 로컬 컨테이너 런타임 | 로컬 컨테이너 런타임 | 지원되지 않음 |
| 프로그래밍 모델 | 장치 및 시스템 독립성 | 웹 및 API 응용 프로그램, 백그라운드 작업을 위한 WebJob | 게스트 실행 파일, 서비스 모델, 작업자 모델, 컨테이너 | 트리거가 있는 함수 | 장치 및 시스템 독립성 | 장치 및 시스템 독립성 | 명령줄 응용 프로그램 |
| 응용 프로그램 업데이트 | 기본 제공 지원 없음 | 배포 슬롯 | 롤링 업그레이드(서비스당) | 배포 슬롯 | 오케스트레이터에 따라 달라짐. 대부분 롤링 업데이트를 지원함 | 컨테이너 이미지 업데이트 | 해당 없음 |
...
| 확장 제한 | 플랫폼 이미지: VMSS당 노드 1000개, 사용자 지정 이미지: VMSS당 노드 100개 | 인스턴스 20개, App Service Environment를 갖는 인스턴스 100개 | VMSS당 노드 100개 | 함수 앱당 인스턴스 200개 | 100 <a href="#note2c"><sup>1</sup></a> |기본적으로 구독당 20개의 컨테이너 그룹입니다. 제한 증가가 필요한 경우 고객 서비스에 문의하세요. <a href="#note3c"><sup>2</sup></a> | 기본적으로 코어 20개로 제한. 제한 증가가 필요한 경우 고객 서비스에 문의하세요. |

메모

2. <span id="note1c">[Container Service 클러스터의 에이전트 노드 확장][scale-acs]</span>을 참조하세요.
3. <span id="note2c">[Azure Container Instances에 대한 할당량 및 지역 가용성](/azure/container-instances/container-instances-quotas)을 참조하세요.</span>
