...
| 엔터티 | 범위 | 길이 | 대/소문자 구분 | 사용할 수 있는 문자 | 제안된 패턴 | 예 |
| --- | --- | --- | --- | --- | --- | --- |
|Virtual Machine |리소스 그룹 |1-15(Windows), 1-64(Linux) |대/소문자 구분하지 않음 |영숫자 및 하이픈 |`<name>-<role>-vm<number>` |`profx-sql-vm1` |
...
Azure Resource Manager는 임의적인 텍스트 문자열로 태그 지정 엔터티를 지원하여 컨텍스트를 식별하고 자동화를 간소화합니다.  예를 들어 `"sqlVersion"="sql2014ee"` 태그는 SQL Server 2014 Enterprise Edition을 실행하는 VM을 식별할 수 있습니다. 태그는 선택한 명명 규칙에 대한 컨텍스트를 보강 및 향상하는 데 사용되어야 합니다.
